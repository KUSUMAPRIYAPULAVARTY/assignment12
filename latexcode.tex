...
\begin{align}
e^k=\sum_{i=0}^\infty \frac{k^i}{i!}\label{1}\\
=1+\frac{k}{1!}+\frac{k^2}{2!}+\frac{k^3}{3!}+\hdots
\end{align}
Representing $e^k$ as a linear combination of basis vectors
\begin{align}
    e^k=\sum_{i=0}^{n-1} c_i\alpha_i\label{2}\\
    \text{where } c_i=\begin{cases}
      \frac{1}{i!}   & \text{if  } \alpha_i=k^i \\
      0 & \text{otherwise}
    \end{cases}
\end{align}
Clearly the set of vectors \cbrak{1,k,k^2,k^3,\hdots\infty} forms a linearly independent set since\\
for $\beta_0,\beta_1,\beta_2,\hdots \in \mathbbf{Q}$
\begin{align}
    \beta_0(1)+ \beta_1(k)+ \beta_2(k^2)+\hdots=0 \\ \iff \beta_0,\beta_1,\beta_2,\hdots=0
\end{align}

Since, \eqref{1} refers to an infinite summation, the representation in \eqref{2} is achieved only if $n$ is $\infty$
