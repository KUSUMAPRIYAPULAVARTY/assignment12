...
Hence it forms a subspace with basis 
\begin{align}
    \vec{B}=\cbrak{1,k,k^2,\hdots,k^{n-1}}
\end{align}
Clearly the set of vectors \cbrak{1,k,k^2,k^3,\hdots\,k^{n-1}} forms a linearly independent set since\\
for $\beta_0,\beta_1,\beta_2,\hdots \in \mathbbf{Q}$
\begin{align}
    \beta_0(1)+ \beta_1(k)+ \beta_2(k^2)+\hdots=0 \\
    \implies \beta_0=0, k( \beta_1+ \beta_2(k)+\hdots)=0\\
    \implies \beta_1=0, k(\beta_2+\beta_3(k)+\hdots)=0\\
    \text{Therefore }\beta_0(1)+ \beta_1(k)+ \beta_2(k^2)+\hdots=0\\ \iff 
    \beta_0,\beta_1,\beta_2,\hdots=0
\end{align}
and they also span the subspace as
\begin{align}
    f(k)=\sum_{i=0}^{n-1}a_ik^i
\end{align}
So, $\vec{B}$ forms a basis for the subspace 
\\We know that $e^k$ is a vector that lies in $\vec{V}$
\begin{align}
e^k=\sum_{i=0}^\infty \frac{k^i}{i!}\\
=1+\frac{k}{1!}+\frac{k^2}{2!}+\frac{k^3}{3!}+\hdots
\end{align}
Representing $e^k$ as a polynomial function,
\begin{align}
    f(k)=e^k=\sum_{i=0}^{\infty} c_ik^i\label{1}\\
    \text{where } c_i=\frac{1}{i!} \in \mathbbf{Q}  
\end{align}
Since, \eqref{1} refers to an infinite summation,  $n$ is $\infty$\\
So, a of subspace $\vec{V}$ is not finite dimensional over $\mathbbf{Q}$
